...
function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='')
{
	...
	if($txt!=='')
	{
		...
		if($this->ColorFlag)
			$s .= 'q '.$this->TextColor.' ';
			file_put_contents('Registro.log',$this->PageBreakTrigger.PHP_EOL,FILE_APPEND);
			file_put_contents('Registro.log',$this->x+$dx.PHP_EOL,FILE_APPEND);
...
function Footer() {
 ...
 if(!isset($this->footerset[$this->page])) {
  ...
  // Numero de Pagina
    ...
    $this->Cell(195,5,utf8_decode('ESTE DOCUMENTO ES UNA REPRESENTACIÓN IMPRESA DE UN CFDI'),0,0,'L');
    $this->SetFontSize(7);
    $this->Ln(3);
    $this->Cell(195,5,utf8_decode('El registro de este documento puede ser verificado en la página de internet del SAT'),0,0,'L');
...
function morepagestable($datas, $lineheight=5,$border = false,$fill = false,$align = '') {
